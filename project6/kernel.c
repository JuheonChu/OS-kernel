...
void handleTimerInterrupt(int segment, int sp) {
  ...
  //Sleep timers for sleeping PCBs

  for(i = 0; i < 8; i++){
    process = &(pcbPool[i]);
    if(process->state == SLEEPING){
      process->sleep--;
     //if time slice expires for sleeping PCBS, put it back to ready queue.
      if(process->sleep <= 0){
	process->state = READY;
	addToReady(process);
      }
  ...
  }while(removeHead != NULL && (removeHead->state ==DEFUNCT || running->state == SLEEPING));
...
int kill(int segment){ //segment index
 ...
 struct PCB * process;
  ...
  //STEP 1: Compute the segment value from given index
...
void sleep(int seconds){
  ...
  running->sleep = temp;
  yield(); //relinquishes the remaining time slice
  restoreDataSegment();
}
